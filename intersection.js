...
function findIntersection(vertex) {
  ...
  const intersectionPrevPoint = intersectRay(
    vertex,
    vertex.bisector,
    prevISectVertex,
    prevISectVertex.bisector
  );

  if (intersectionPrevPoint !== undefined) {
    intersectionPrevDistToEdge = distToSegment(
      intersectionPrevPoint,
      vertex,
      nextVertex
    );
    intersectionPrev = new IntersectionPoint(
      intersectionPrevPoint,
      [prevISectVertex, vertex],
      intersectionPrevDistToEdge
    );
  }

  let intersectionNext;
  let intersectionNextDistToEdge;
  const intersectionNextPoint = intersectRay(
    vertex,
    vertex.bisector,
    nextISectVertex,
    nextISectVertex.bisector
  );
  if (intersectionNextPoint !== undefined) {
    intersectionNextDistToEdge = distToSegment(
      intersectionNextPoint,
      vertex,
      nextVertex
    );
    intersectionNext = new IntersectionPoint(
      intersectionNextPoint,
      [vertex, nextISectVertex],
      intersectionNextDistToEdge
    );
  }
